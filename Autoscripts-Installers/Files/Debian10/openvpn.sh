...
wget -O /etc/openvpn/easy-rsa/vars "https://raw.githubusercontent.com/apidotmy/Autoscript/main/Files/Debian10/vars.conf"
# edit projek export KEY_NAME="redssh"
...
# buat server key name yang telah kita buat sebelum nya yakni "redssh"
./build-key-server redssh
...
cert easy-rsa/keys/redssh.crt
key easy-rsa/keys/redssh.key
dh dh2048.pem
...
cp /etc/openvpn/easy-rsa/keys/{redssh.crt,redssh.key,ca.crt,ta.key} /etc/openvpn
ls /etc/openvpn
...
mkdir clientconfig
cp /etc/openvpn/easy-rsa/keys/{redssh.crt,redssh.key,ca.crt,ta.key} clientconfig/
cd clientconfig
